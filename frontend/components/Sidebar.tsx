          ...
          {history.length === 0 ? (
            ...
            history.map((item) => (
              <div
                ...
                className="group relative p-3 hover:bg-gray-800 rounded-lg cursor-pointer transition-colors border border-transparent hover:border-gray-700 pr-10" // <-- pr-10 ekledik (ikon için yer açtık)
              >
                ...
                {/* SİLME BUTONU (Sadece üzerine gelince görünür: group-hover) */}
                <button
                  onClick={(e) => {
                    e.stopPropagation(); // <-- ÖNEMLİ: Satıra tıklamış gibi davranma!
                    onDelete(item.id);
                  }}
